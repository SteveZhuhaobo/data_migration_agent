...
def load_config():
    ...
    try:
        ...
        # Validate required Snowflake configuration fields
        if 'snowflake' not in config:
            raise ValueError("Missing 'snowflake' section in configuration")
            
        snowflake_config = config['snowflake']
        required_fields = ['account', 'user']
        
        for field in required_fields:
            if field not in snowflake_config or not snowflake_config[field]:
                raise ValueError(f"Missing required Snowflake configuration field: {field}")
        
        # Check authentication method
        has_password = 'password' in snowflake_config and snowflake_config['password']
        has_private_key = 'private_key_path' in snowflake_config and snowflake_config['private_key_path']
        
        # Try environment variables for password
        ...
        if not has_password and not has_private_key:
            raise ValueError("Missing authentication method: provide either 'password' or 'private_key_path' in configuration, or set SNOWFLAKE_PASSWORD environment variable")
        
        # Set default values if not provided
        if 'timeout' not in snowflake_config:
            snowflake_config['timeout'] = 120
        if 'max_retries' not in snowflake_config:
            snowflake_config['max_retries'] = 3
        if 'retry_delay' not in snowflake_config:
            snowflake_config['retry_delay'] = 5
        if 'pool_size' not in snowflake_config:
            snowflake_config['pool_size'] = 5
        if 'pool_timeout' not in snowflake_config:
            snowflake_config['pool_timeout'] = 30
...
async def ping_tool() -> str:
    ...
    return json.dumps({
        "success": True,
        "message": "Snowflake MCP Server is responsive",
