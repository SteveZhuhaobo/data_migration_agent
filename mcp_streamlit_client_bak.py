                        ...
                        if message.tool_calls:
                            for tool_call in message.tool_calls:
                                ...
                                if tool_response and "result" in tool_response:
                                    ...
                                    # Format SQL query results in a human-readable way
                                    if isinstance(result, dict) and "data" in result and "columns" in result:
                                        if result["data"]:
                                            ...
                                            if len(result["data"]) == 1 and "row_count" in result["data"][0]:
                                                row = result["data"][0]
                                                summary = f"The table with the most records is **{row.get('schema_name', '')}.{row.get('table_name', '')}** with **{row.get('row_count', 0):,}** records."
                                                st.write(summary)
                                                assistant_message = summary
                                            else:
                                                assistant_message = f"Query returned {len(result['data'])} rows."
                                        else:
                                            st.write("No data returned from the query.")
                                            assistant_message = "No data returned from the query."
                                        ...
                                        st.json(result)
                                        assistant_message = f"Query executed successfully. Result: {json.dumps(result, indent=2)}"
