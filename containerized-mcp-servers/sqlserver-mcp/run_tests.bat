...
docker-compose -f docker-compose.test.yml run --rm sqlserver-mcp-test python -c "import unittest; import sys; import os; sys.path.insert(0, '/app'); print('Running basic integration validation...'); import server; import env_validator; import health_check; print('✅ All modules imported successfully'); os.environ.update({'SQLSERVER_SERVER': 'localhost', 'SQLSERVER_USERNAME': 'test_user', 'SQLSERVER_PASSWORD': 'test_password'}); server.load_config(); print('✅ Configuration loaded successfully'); print('🎉 Basic integration tests passed!')"
