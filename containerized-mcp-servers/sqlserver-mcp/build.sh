...
# SQL Server MCP Server Build Script
# This script builds the SQL Server MCP Docker container

set -e  # Exit on any error

# Configuration
IMAGE_NAME="sqlserver-mcp"
IMAGE_TAG="${1:-latest}"
REGISTRY="${REGISTRY:-}"
...
build_image() {
    ...
    log_info "Building SQL Server MCP Server Docker image..."
...
show_usage() {
    ...
    echo ""
    echo "Build the SQL Server MCP Server Docker image"
    echo ""
    echo "Arguments:"
...
# Main execution
main() {
    log_info "Starting SQL Server MCP Server build process..."
    ...
    echo "  3. Run the container: docker run -e SQLSERVER_SERVER=... -e SQLSERVER_USERNAME=... -e SQLSERVER_PASSWORD=... ${REGISTRY:+$REGISTRY/}${IMAGE_NAME}:${IMAGE_TAG}"
}
