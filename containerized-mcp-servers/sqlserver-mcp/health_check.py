#!/usr/bin/env python3
"""
Health check and monitoring for SQL Server MCP Server
"""
...
class HealthChecker:
    ...
    async def perform_health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            permissions_check = await self._check_database_permissions()
            health_status["checks"]["database_permissions"] = permissions_check
    ...
    async def _check_authentication(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Execute test_connection which includes auth check
            result_json = await server.test_connection()
    ...
    async def _check_query_execution(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result_json = await server.execute_query("SELECT GETDATE() as current_time")
            result = json.loads(result_json)
    ...
    async def _check_database_permissions(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cursor.execute("""
                SELECT 
                    ...
                    HAS_PERMS_BY_NAME(DB_NAME(), 'DATABASE', 'SELECT') as select_permission,
                    HAS_PERMS_BY_NAME(DB_NAME(), 'DATABASE', 'INSERT') as insert_permission,
                    HAS_PERMS_BY_NAME(DB_NAME(), 'DATABASE', 'CREATE TABLE') as create_table_permission
            """)
            ...
            permissions_dict = {
                "view_server_state": bool(permissions[0]) if permissions[0] is not None else False,
                "select_permission": bool(permissions[1]) if permissions[1] is not None else False,
                "insert_permission": bool(permissions[2]) if permissions[2] is not None else False,
                "create_table_permission": bool(permissions[3]) if permissions[3] is not None else False
            }
            
            # Consider healthy if at least select permission is available
            healthy = permissions_dict.get("select_permission", False)
