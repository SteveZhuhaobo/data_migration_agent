...
echo "🧪 Running Databricks MCP Server Tests"
echo "======================================"
...
docker-compose -f docker-compose.test.yml run --rm databricks-mcp-test python -m pytest test_server.py -v --tb=short --junit-xml=test_results/unit_tests.xml

echo "🔗 Running integration tests..."
# Note: Integration tests require Docker-in-Docker or Docker socket mounting
# For now, we'll run them in a simpler way
docker-compose -f docker-compose.test.yml run --rm databricks-mcp-test python -c "
import unittest
import sys
import os
