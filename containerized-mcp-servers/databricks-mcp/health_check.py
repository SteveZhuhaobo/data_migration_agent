#!/usr/bin/env python3
"""
Health check and monitoring for Databricks MCP Server
"""
...
class HealthChecker:
    ...
    async def perform_health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check 2: Warehouse status
            warehouse_check = await self._check_warehouse_status()
            health_status["checks"]["warehouse_status"] = warehouse_check
            
            # Check 3: Database connectivity
            db_check = await self._check_database_connection()
            health_status["checks"]["database_connection"] = db_check
            
            # Check 4: REST API connectivity
            api_check = await self._check_rest_api()
            health_status["checks"]["rest_api"] = api_check
            
            # Check 5: Basic query execution
    ...
    async def _check_warehouse_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check warehouse status
            warehouse_ok, warehouse_msg = server.check_warehouse_status()
            
            return {
                "healthy": warehouse_ok,
                "details": {
                    "message": warehouse_msg,
                    "warehouse_check": "passed" if warehouse_ok else "failed"
                }
            }
    ...
    async def _check_rest_api(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = session.get(f"{base_url}/api/2.0/clusters/list", timeout=30)
            
            return {
                "healthy": response.status_code < 400,
                "details": {
                    "status_code": response.status_code,
                    "api_test": "passed" if response.status_code < 400 else "failed"
                }
            }
    ...
    async def _check_query_execution(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result_json = await server.execute_query("SELECT 1 as test_value")
    ...
    async def validate_connection_with_retry(self) -> Tuple[bool, str]:
        ...
        max_retries = 3
        retry_delay = 10  # seconds (longer for Databricks due to cold starts)
            ...
            try:
                ...
                if not warehouse_ok:
                    if attempt < max_retries - 1:
                        ...
                        return False, f"Warehouse check failed: {warehouse_msg}"
